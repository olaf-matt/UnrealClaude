...
public:
	virtual FMCPToolInfo GetInfo() const override
	{
		...
		Info.Parameters = {
			...
			FMCPToolParameter(TEXT("node_params"), TEXT("object"), TEXT("Node params. GetVariable: {\"variable_name\":\"MyVar\"}. Comparison nodes have no required params (set defaults via set_pin_default_value)."), false),
			FMCPToolParameter(TEXT("node_id"), TEXT("string"), TEXT("Node ID from get_transition_nodes result. Required for delete_condition_node and inspect_node_pins."), false),
			FMCPToolParameter(TEXT("source_node_id"), TEXT("string"), TEXT("Source node ID (output side of connection). Use get_transition_nodes to find node IDs."), false),
			FMCPToolParameter(TEXT("source_pin"), TEXT("string"), TEXT("Output pin name on source node. Boolean output pin is typically named after the node type (e.g., 'Result' on Greater). Use inspect_node_pins to verify."), false),
			FMCPToolParameter(TEXT("target_node_id"), TEXT("string"), TEXT("Target node ID (input side of connection)."), false),
			FMCPToolParameter(TEXT("target_pin"), TEXT("string"), TEXT("Input pin name on target node (e.g., 'A', 'B' on comparison nodes; 'Condition' on result node). Use inspect_node_pins to verify."), false),
